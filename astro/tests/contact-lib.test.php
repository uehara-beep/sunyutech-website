...
// ---- ログに残すIPの丸め --------------------------------------------------
//
// エラーログには生のIPを書かない。ログは提供事業者のローテーション対象外で
// 際限なく残るため、プライバシーポリシーの保管期間と食い違ってしまう。
// 障害調査に足る粒度だけ残す。

echo "\ncf_mask_ip\n";

test('IPv4 は末尾を伏せる', function () {
    assert_same('203.0.113.x', cf_mask_ip('203.0.113.50'));
...
test('元のIPが復元できないこと', function () {
    // 同じ /24 の別ホストが同じ表記になる = 個人の特定に使えない
    assert_same(cf_mask_ip('203.0.113.1'), cf_mask_ip('203.0.113.254'));
});

test('IPとして不正な値は伏せ字にする', function () {
    assert_same('(不明)', cf_mask_ip('garbage'));
    assert_same('(不明)', cf_mask_ip(''));
});
